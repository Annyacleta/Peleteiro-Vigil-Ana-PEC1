...
# 1. Archivos mzML
files <- list.files("C:/Users/pele_/Desktop/ST003680_Rawfiles/centroiding", pattern = ".mzML", full.names = TRUE)

# 2. Carga en modo eficiente
raw_data <- readMSData(files, mode = "onDisk")

# 3. Crear objeto de procesamiento
p <- CentWaveParam(ppm = 15, peakwidth = c(5, 20), snthresh = 10)
xdata <- findChromPeaks(raw_data, param = p)

# 4. Alineamiento y agrupamiento
sample_groups <- rep(1, length(fileNames(xdata))) #agrupo ratones según microbiota recibida (KC o SC)
...
# 6. Anotación con CAMERA
xset <- as(xdata, "xcmsSet")
anotado <- xsAnnotate(xset)
anotado <- groupFWHM(anotado)
anotado <- findIsotopes(anotado)
anotado <- findAdducts(anotado, polarity = "positive")

```

```{r}
#Creación de objeto SummarizedExperiment
# COMPROBAR QUE RDA OBTENIDO EN REPO GITHUB

library(SummarizedExperiment)

# 1. Extraer matriz de intensidades
matriz <- featureValues(xdata)

# 2. Metadatos de muestras (puedes cargar desde .csv si no tienes)
metadatos_muestras <- read.csv("sample_metadata.csv", row.names = 1)

# 3. Metadatos de picos / features (m/z, RT)
metadatos_picos <- as.data.frame(featureDefinitions(xdata))

# 4. Crear el SummarizedExperiment
se <- SummarizedExperiment(
  assays = list(intensities = as.matrix(matriz)),
  colData = metadatos_muestras,
  rowData = metadatos_picos
)
